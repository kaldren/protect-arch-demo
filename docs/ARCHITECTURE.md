...
- Interfaces in `Domain/Interfaces/` must only contain interfaces — no classes.

## Architecture Tests

All dependency rules are enforced by automated tests in `tests/ArchitectureTests/` using **ArchUnitNET**.

```bash
dotnet test tests/ArchitectureTests
```

Every forbidden dependency pair in the table above has a corresponding test. If you add a new layer or project, add matching tests.
